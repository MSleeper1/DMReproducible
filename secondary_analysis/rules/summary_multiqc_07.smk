# Multiqc rule for compiling reports from post merge and methylation extraction
...
#         expand("{root}/{rep_dir}/06_merged_bismark_deduplication/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.deduplication_report.txt", sample = sample_info.itertuples(), root = config["root"], rep_dir=config["reports_dir"]),
#         expand("{root}/{rep_dir}/06_merged_deduped_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.log", root = config["root"], sample = sample_info.itertuples(), rep_dir=config["reports_dir"]),
#         expand("{root}/{data_dir}/07_bismark_methyl_extractor/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.M-bias.txt", sample = sample_info.itertuples(), root = config["root"], data_dir=config["data_dir"]),
#         expand("{root}/{data_dir}/07_bismark_methyl_extractor/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}_splitting_report.txt", sample = sample_info.itertuples(), root = config["root"], data_dir=config["data_dir"]),
#         expand("{root}/{data_dir}/07_bismark_methyl_extractor/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.cytosine_context_summary.txt", sample = sample_info.itertuples(), root = config["root"], data_dir=config["data_dir"]),
#         expand("{root}/{data_dir}/07_methyldackel_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}_CpG.bedGraph", sample = sample_info.itertuples(), root = config["root"], data_dir=config["data_dir"]),
#         expand("{root}/{data_dir}/07_methyldackel_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}_mergedContext_CpG.bedGraph", sample = sample_info.itertuples(), root = config["root"], data_dir=config["data_dir"]),
#         expand("{root}/{data_dir}/07_methyldackel_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}_CpG.methylKit", sample = sample_info.itertuples(), root = config["root"], data_dir=config["data_dir"]),
#         expand("{root}/{data_dir}/07_methyldackel_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.cytosine_report.txt", sample = sample_info.itertuples(), root = config["root"], data_dir=config["data_dir"])
#     ]
...
#---------------------------------------------------------------
rule multiqc_compile_bwa_reports_07:
    input:
        expand("{root}/{rep_dir}/06_feature_counts_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.featureCounts{suf}", root = config["root"], rep_dir=config["reports_dir"], suf=["", ".summary", ".jcounts"], sample = sample_info.itertuples()), # featureCounts bwa
        expand("{root}/{rep_dir}/06_fastqc_post_merge_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}_fastqc.{suf}", root = config["root"], rep_dir = config["reports_dir"], suf=["html","zip"], sample = sample_info.itertuples()), # fastqc report bwa
        expand("{root}/{rep_dir}/06_samtools_post_merge_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.bam.stats", root = config["root"], rep_dir=config["reports_dir"], sample = sample_info.itertuples()), # samtools stats report bwa
        ...
        expand("{root}/{rep_dir}/06_qualimap_bwa/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}", root = config["root"], rep_dir=config["reports_dir"], sample = sample_info.itertuples()), # qualimap report bwa
    output:
        file = expand("{root}/{rep_dir}/summary/{{ref}}--07_final_multiqc_bwa.html", sample = sample_info.itertuples(), root = config["root"], rep_dir=config["reports_dir"]),
        input_list = expand("{root}/{rep_dir}/summary/{{ref}}--07_final_qc_report_list_bwa.txt", sample = sample_info.itertuples(), root = config["root"], rep_dir=config["reports_dir"]),
        multiqc_command = expand("{root}/{rep_dir}/summary/{{ref}}--07_multiqc_command_bwa.sh", sample = sample_info.itertuples(), root = config["root"], rep_dir=config["reports_dir"])
    log:
        "logs/secondary_rules/multiqc/{ref}--07_multiqc_bwa.log"
    conda:
        "../../environment_files/multiqc.yaml"
    wildcard_constraints:
        ref = "|".join(sample_info["ref"].tolist())
    params:
        output_filename = "{ref}--07_final_multiqc_bwa", # Required: do not change without adjusting the rule output
        ...
        report_title = "07 Final QC Reports for Secondary Analysis for dataset ref:{ref}", # Optional: title for multiqc report (can be changed freely)
...
rule multiqc_compile_bis_reports_07:
    input:
        expand("{root}/{rep_dir}/06_feature_counts_bis/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.featureCounts{suf}", root = config["root"], rep_dir=config["reports_dir"], suf=["", ".summary", ".jcounts"], sample = sample_info.itertuples()), # featureCounts bis
        expand("{root}/{rep_dir}/06_fastqc_post_merge_bis/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}_fastqc.{suf}", root = config["root"], rep_dir = config["reports_dir"], suf=["html","zip"], sample = sample_info.itertuples()), # fastqc report bis
        expand("{root}/{rep_dir}/06_samtools_post_merge_bis/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}.bam.stats",  root = config["root"], rep_dir=config["reports_dir"], sample = sample_info.itertuples()), # samtools stats report bis
        ...
        # expand("{root}/{rep_dir}/06_qualimap_bis/{{ref}}--{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}", root = config["root"], rep_dir=config["reports_dir"], sample = sample_info.itertuples()), # qualimap report bis
    output:
        file = expand("{root}/{rep_dir}/summary/{{ref}}--07_final_multiqc_bis.html", sample = sample_info.itertuples(), root = config["root"], rep_dir=config["reports_dir"]),
        input_list = expand("{root}/{rep_dir}/summary/{{ref}}--07_final_qc_report_list_bis.txt", sample = sample_info.itertuples(), root = config["root"], rep_dir=config["reports_dir"]),
        multiqc_command = expand("{root}/{rep_dir}/summary/{{ref}}--07_multiqc_command_bis.sh", sample = sample_info.itertuples(), root = config["root"], rep_dir=config["reports_dir"])
    log:
        "logs/secondary_rules/multiqc/{ref}--07_multiqc_bis.log"
    conda:
        "../../environment_files/multiqc.yaml"
    wildcard_constraints:
        ref = "|".join(sample_info["ref"].tolist())
    params:
        output_filename = "{ref}--07_final_multiqc_bis", # Required: do not change without adjusting the rule output
        outdir = config["root"] + "/" + config["reports_dir"] + "/summary", # Required: do not change without adjusting the rule output
        comment = "Multiqc report for files post merge and methylation extraction. Includes reports from fastqc, samtools, mosdepth featurecounts... Reports compiled by snakemake DMR_workflow pipeline.", # Optional: comment for multiqc report (can be changed freely)
        report_title = "07 Final QC Reports for Secondary Analysis for dataset ref:{ref}", # Optional: title for multiqc report (can be changed freely)
        extra = "--verbose"  # Optional: extra parameters for multiqc (can be changed freely)
